...
def registration_user(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                'Спасибо за регистрацию!'.format(user.username)
            )
            return HttpResponseRedirect(reverse('accounts:login'))
    else:
        form = UserRegistrationForm()
    context = {'form': form}
    return render(request, 'htmlpage/accounts/registration.html', context)
...
def login_user(request):
    ...
    return render(request, 'htmlpage/accounts/login.html', {})
...
def edit_profile(request, pk):
    ...
    return render(request, 'htmlpage/accounts/edit_profile.html', context)
...
def profile(request):
    ...
    return render(request, 'htmlpage/accounts/registration_next.html', context)


# Создана функция для отображения личного кабинета пользователя на html странице
@login_required
def personal_cabinet(request):
    return render(request, 'htmlpage/accounts/personal_cabinet.html')
...
def user_publication(request):
    user_pub = Publication.objects.filter(author=request.user)
    context = {'user_pub': user_pub}
    return render(request, 'htmlpage/accounts/user_publication.html', context)
